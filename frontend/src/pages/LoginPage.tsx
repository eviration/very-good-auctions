...
export default function LoginPage() {
  ...
  const handleProviderLogin = async (providerId?: string) => {
    ...
    try {
      // For Microsoft Entra External ID (CIAM), use the 'idp' parameter
      // to go directly to a specific identity provider
      // The provider ID must match the name configured in Azure portal
      const extraQueryParameters: Record<string, string> = {}

      if (providerId) {
        // 'idp' parameter tells Entra External ID to skip the IdP selection
        // and go directly to the specified provider
        // Common values: 'google.com', 'apple.com', 'facebook.com', 'MicrosoftAccount'
        extraQueryParameters.idp = providerId
      }
  ...
  // The 'hint' values must match the IdP names configured in Azure Entra External ID
  // Go to Azure Portal > External Identities > All identity providers to see the exact names
  const providers = [
    {
      id: 'google',
      name: 'Google',
      hint: 'Google',  // Use the exact name from Azure portal
      icon: (
    ...
    },
    {
      id: 'apple',
      name: 'Apple',
      hint: 'Apple',  // Use the exact name from Azure portal
      icon: (
    ...
    },
    {
      id: 'microsoft',
      name: 'Microsoft',
      hint: 'MicrosoftAccount',  // MSA personal accounts
      icon: (
        <svg className="w-5 h-5" viewBox="0 0 24 24">
    ...
    },
    {
      id: 'facebook',
      name: 'Facebook',
      hint: 'Facebook',  // Use the exact name from Azure portal
      icon: (
