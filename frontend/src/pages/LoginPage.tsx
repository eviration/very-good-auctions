...
export default function LoginPage() {
  ...
  const handleProviderLogin = async (providerHint?: string) => {
    setIsRedirecting(true)
    try {
      // For Microsoft Entra External ID, we can use domain_hint to suggest a provider
      // The actual provider configuration must be set up in Azure portal
      const extraQueryParameters: Record<string, string> = {}

      if (providerHint) {
        // domain_hint tells Entra which IdP to prefer
        extraQueryParameters.domain_hint = providerHint
      }
