...
### 2.4 Individual Auctions (Updated)
...
### 3.7 Organization Payout Flow

```
Buyers Pay (total $1,000 for event)
    ↓
Stripe Takes: ~$32.00 (2.9% + $0.30 per transaction)
    ↓
Platform Fee Already Paid: $35 (medium tier)
    ↓
Organization Receives: ~$968 via Stripe Connect
    ↓
Payout per Stripe Connect schedule
```
...
// Run daily at 2 AM
- processDailyPayouts()       // PayPal batch payouts
...
Estimated implementation: 10 sprints (5-10 weeks)
